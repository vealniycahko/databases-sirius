...
<br>APP_NAME, APP_PORT, POSTGRES_HOST, POSTGRES_PORT, POSTGRES_DB, POSTGRES_USER, POSTGRES_PASSWORD, REDIS_HOST, REDIS_PORT, REDIS_PASSWORD, REPLICA_HOST, SLAVE_PORT, ELASTIC_HOST, ELASTIC_PORT, CLICKHOUSE_HOST, CLICKHOUSE_USER, CLICKHOUSE_PASSWORD, CLICKHOUSE_PORT
<br>переменные  окружения для реплики:
<br>SLAVE_PORT, MASTER_PORT, MASTER_HOST, MASTER_PASSWORD
```bash
